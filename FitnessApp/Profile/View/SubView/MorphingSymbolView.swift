...
struct MorphingSymbolView: View {
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            if let rendererImage = ctx.resolveSymbol(id: 0) {
                ctx.draw(rendererImage, at: CGPoint(x: size.width / 2, y: size.height / 2))
            }
        } symbols: {
            ImageView()
                .tag(0)
...
#Preview {
    MorphingSymbolView(symbol: "gearshape.fill", config: .init(font: .system(size: 100, weight: .bold), frame: CGSize(width: 250, height: 200), radius: 15, foregroundColor: Color.fitnessGreenMain))
}
